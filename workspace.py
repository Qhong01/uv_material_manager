...
def sync_layouts_to_scene(scene):
    ...
    if not scene:
        return
    if not hasattr(scene, 'um_custom_layouts'):
        return
...
def capture_area_sidebar_state(context, area, key):
    ...
    try:
        ...
        if active_space:
            ...
            if hasattr(active_space, 'use_image_pin'):
                state_dict['use_image_pin'] = active_space.use_image_pin
    ...
    def apply_cat():
        ...
        try:
            ...
            if act_space and act_space.active:
                ...
                if hasattr(sp, 'use_image_pin') and 'use_image_pin' in state:
                    try:
                        sp.use_image_pin = bool(state['use_image_pin'])
                    except Exception:
                        pass
                # 处理资产浏览器参数
                if is_asset_mode:
...
def execute_toggle_layout(self_op, context, item_id="", item_name="", editor_type="", index=-1):
    ...
    # 注意：如果刚才执行了 back_to_previous，context.area 已经失效 (属于被销毁的 TEMP 屏幕)，不能再使用它
    orig_3d_area = None
    if not did_un_fullscreen and context.area and context.area.type == 'VIEW_3D':
        try:
            # 简单测试一下 area 是否存活
            _ = context.area.width
            orig_3d_area = context.area
    ...
    def perform_split_and_setup():
        try:
            # 重新获取最新的安全上下文，防止定时器执行时原指针失效
            cur_window, cur_screen = get_safe_window_and_screen(bpy.context)
            if not cur_screen:
                return

            cur_area = None
            view3d_areas = [a for a in cur_screen.areas if a.type == 'VIEW_3D']
            if view3d_areas:
                cur_area = max(view3d_areas, key=lambda a: a.width * a.height)
            ...
            areas_before = list(cur_screen.areas)
            ...
            new_area = None
            for a in cur_screen.areas:
                if a not in areas_before:
                    new_area = a
                    break
            if not new_area:
                new_area = cur_screen.areas[-1]

            if split_dir == 'VERTICAL':
                left_a = cur_area if cur_area.x < new_area.x else new_area
                right_a = new_area if cur_area.x < new_area.x else cur_area
                secondary_area = left_a if direction == 'LEFT' else right_a
                main_3d_area = right_a if direction == 'LEFT' else left_a
            else:
                bottom_a = cur_area if cur_area.y < new_area.y else new_area
                top_a = new_area if cur_area.y < new_area.y else cur_area
            ...
            restore_area_sidebar_state(bpy.context, secondary_area, target_item.id, target_item.editor_type)
            if main_3d_cat or main_3d_show_ui is not None:
                restore_view3d_sidebar_state(bpy.context, main_3d_area, main_3d_cat, main_3d_show_ui)
    ...
    if did_un_fullscreen:
        # 如果刚退出了全屏，主屏幕的布局需要在下一帧才能完全更新和允许被分割，必须延迟执行
        bpy.app.timers.register(perform_split_and_setup, first_interval=0.01)
        return {'FINISHED'}
    else:
        perform_split_and_setup()
        return {'FINISHED'}


class WORKSPACE_OT_ToggleCustomLayout(Operator):
    bl_idname = "screen.toggle_custom_layout"
    bl_label = "切换分屏视图"
    bl_description = "智能切换该分屏视图（全局通用快捷键：在3D视图或左侧副屏任意窗口按下均可切换/关闭）"
...
def make_slot_operator(slot_idx):
    ...
    class WORKSPACE_OT_ToggleSlotLayout(Operator):
        bl_idname = f"screen.toggle_custom_layout_{slot_num}"
        bl_label = f"切换分屏视图项 {slot_num}"
        bl_description = f"智能切换第 {slot_num} 个自定义分屏视图（全局快捷键：鼠标停在任意区域均可生效）"
        bl_options = {'REGISTER', 'UNDO'}
...
def make_type_operator(editor_type, type_name):
    class WORKSPACE_OT_ToggleTypeLayout(Operator):
        bl_idname = f"screen.toggle_split_{editor_type.lower()}"
        bl_label = f"切换 {type_name} 分屏"
        bl_description = f"智能切换 {type_name} 分屏视图（全局快捷键支持）"
        bl_options = {'REGISTER', 'UNDO'}
