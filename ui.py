...
class VIEW3D_PT_UltimateManager(Panel):
    ...
    def draw(self, context):
        ...
        # 新增第二排建模与选择工具栏 (智能环选、90°旋转)
        tools_row2 = layout.row(align=True)
        tools_row2.operator("mesh.smart_loop_select", text="智能环选", icon='MOD_EDGESPLIT')
        tools_row2.operator("view3d.open_rotate_90_menu", text="90°旋转", icon='DRIVER_ROTATIONAL_DIFFERENCE')
        ...
        if len(custom_layouts) == 0:
            ...
            for idx, item in enumerate(custom_layouts):
                ...
                slot_num = idx + 1
                if slot_num <= 10:
                    op_id = f"screen.toggle_custom_layout_{slot_num}"
                else:
                    op_id = "screen.toggle_custom_layout"

                btn = top_row.operator(op_id, text=item.name, icon=icon_name, depress=is_active)
                if op_id == "screen.toggle_custom_layout":
                    btn.item_id = item.id
